...
def train_scover_bs(config, train_data, val_data, max_epochs=30, num_gpus=1):
    if num_gpus > 0:
...
def train_scover(config, train_loader, val_loader, max_epochs=30, num_gpus=1):
    if num_gpus > 0:
        assert torch.cuda.is_available(
        ), "num_gpus > 0 but CUDA is not available"
    seq_length = train_loader.dataset[0][0].shape[1]
