...
setwd("~/scanem_pytorch/data_gen/FigureGithub/DownstreamAnalysis/")
```

## Introduction

This vignette focusses on downstream analysis of the output from **scover**. After running
**scover**, it outputs different types of information. The most important are:

1. The alignments of the found convolutional kernels to a database of motifs, with significance (q-)scores for each alignment
2. The influence scores of the convolutional kernels in neural network, representing the change in prediction when a particular motif is left out

The central part of **scover** trains 10 different neural network models, each using a 
different 90% of the dataset for training and the remaining 10% for testing. Each model contains (by default) 
300 convolutional filters, so there are a total of 3,000 motifs. To ensure that the results are robust, 
**scover** seeks to identify motif clusters that show up repeatedly, therefor we require that a motif 
...
Then it becomes possible to select reproducible motifs. For a visual overview, see the figure below. 

One of the outputs from the model is a HDF5 file containing motif influence scores 
for each model. These are calculated for each convolutional filter in each pool - these
scores represent the mean change in prediction for a particular pool when a convolutional filter is left out.

Below is a visual overview of the workflow for motif analysis 
using **scover**. The downstream analysis described in this section starts at the third step, 
where we have obtained influence scores from the models and the motif alignments from Tomtom. 

<center>
![scover_output_workflow](scover_motif_analysis_workflow.png)
</center>
...
First, I'll use the custom functions to read the data, tomtom output, and the 
network influence scores. Then I'll filter the tomtom output to only include TFs that 
were expressed 
...
Now I'll generate a motif alignment graph (as in the other guide) and plot the
graph:
...
reproducibility, and motif cluster annotation (only the clusters that aligned
                                               to known motifs will be included in this analysis):
...
This part is the same as in the 
...
Here, every row corresponds to a convolutional filter and every column corresponds to 
a cell type. 

As you can see, a small subset of the motifs has the largest impact. If we 
...
"cell-type-specific" a motif influence score really is. For this, the 
"motif cell type entropy" score has been calculated for each motif, which is the 
normalised Shannon entropy of a motif influence score across cell types. The lower this 
score is, the further from a uniform distribution the influence score distribution is. 
...
To get an overview at the "cell type category" level, rather than the individual
cell type level, you can do some other types of analysis. For this, I've created 
(for this specific example) a named vector with the different cell types as 
names that contains the corresponding cell type category:
...
over the steps in this analysis, I would recommend looking at the corresponding
...
the different plots that show how the expression and influence values are related. 
...
one of them the more likely candidate. 
...
```{r update_cluster_annotations, fig.height = 12, fig.width = 12}
...
First, I'll do the one based on motif influence scores:
...
I'll add cell type and cell type category annotation to this.
...
mature kidney. I'll add annotation for the origin of the pools this way (but 
...
Now, I'll plot the PCA plot, showing the cell type categories as colours, and 
...
Now, I'll do exactly the same, but based on expression values:
