...
Load your `SingleCellExperiment` object. In this case I am using a SNARE-seq dataset with cells from the adult mouse visual cortex:
...
In addition, it's good practice to exclude cells that have an "unknown" cell type. 
...
Pooling cells together for **scover** is a trade off between getting rid of undesired zeroes in your count
matrix and retaining variability between cells and cell types. For the datasets that I have worked with so far, a pool size of 100 cells per pool is good, as it reduces the sparsity below 70%. However, I recommend that you try a couple of different settings since the number of remaining pools and the sparsity is dependent on your dataset. Sometimes it is impossible to reduce the sparsity below 70% - in that case, I recommend that you try a couple of different pool sizes to see what works best. 
...
Note that for pool sizes beyond a particular size, you lose cell types as they have fewer than `pool_size` cells. 
...
This part of the workflow will only work if your rownames of the `SingleCellExperiment` object
contain the peak regions in this format: `chromosome_peakstart_peakend`, so, for example,
`chr1_283990_284520`. If the peak regions are instead using different symbols to
separate the different parts (for instance, `chromosome:peakstart-peakend`) then
it becomes necessary to update the regular expressions found in the code block. 
(For the example `chromosome:peakstart-peakend`, the updated first line then becomes
`peak_start <- as.numeric(str_remove(str_remove(peaknames, "^chr[a-zA-Z0-9]+:"), "-[0-9]+$"))`,
for example).

In most peak regions, TFs occupy sequences around the middle of the peak. 
To decrease runtimes and increase signal/noise, it is useful to take the 
